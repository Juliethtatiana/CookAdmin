...
public class Cargador {
       ...
       File[] files = f.listFiles(         
            new FileFilter() {                
                public boolean accept(File fileToBeFiltered) { 
                    //System.out.println("files: "+fileToBeFiltered.getName());
        ...
        for (int i = 0; i < files.length ; i++) {
            try {                
                ...
                System.out.println("nc:" +nc+ " ncbc: "+ncbc);
                ////////////////////////////////////////////////////////////
                InfoComponente ip=new InfoComponente(nc,ncbc); 
                urls.add(files[i].toURI().toURL());
                System.out.println("nc:" +nc+ " ip: "+ip);   
	   ...
	   Class clase = null;
		try {
			if(n<listaDeCargadoresRegistro.size()){
                            System.out.println("nombre: "+nombre+" lista cargadores registro "+listaDeCargadoresRegistro.get(n).registroDeComponentes.get(nombre));
   ...
   public Class getClase(String nombreDeLaClase){
    ...
    public List<CargadorRegistro> getListaDeCargadoresRegistro() {
        return listaDeCargadoresRegistro;
    }
